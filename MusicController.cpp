...
void MusicController::generateMidGameMusicList() {
	...
	while (i < 6) {
		...
		if (!already_in_numbs) {
			...
			string music_path = base_path;
			music_path.append(game_play_songs[r]);

			Mid_game_music_list.insertAtEnd(make_shared<Node<GamingMusic>>(make_shared<GamingMusic>(GamingMusic(LoadMusicStream(music_path.c_str()), Master_volume))));
		}
	}
}
...
void MusicController::generateMusic() {
	string main_music_path = base_path;
	main_music_path.append("audio/main_screen.mp3");

	Main_screen_music = make_shared<Node<GamingMusic>>(make_shared<GamingMusic>(GamingMusic(LoadMusicStream(main_music_path.c_str()), Master_volume)));
